...
#include <string.h>
#include <libcss/libcss.h>
void resolve_url_auto(void *pw, const char *base, lwc_string *rel, lwc_string **abs) { *abs = lwc_string_ref(rel); }
void font_name_auto(void *pw, const char *fm, lwc_string **res) { lwc_string_intern(fm, strlen(fm), res); }
...
static void test_gap() {
    ...
    css_stylesheet_params params = {
        ...
        .allow_quirks = false,
        .inline_style = false,
        .resolve = resolve_url_auto,
        .resolve_pw = NULL,
        .import = NULL,
        .import_pw = NULL,
        .color = NULL,
        .color_pw = NULL,
        .font_name = font_name_auto,
        .font_name_pw = NULL
    };
    css_error err = css_stylesheet_create(&params, &sheet);
    if (err) return;

    const char *css = "div { gap: 16px; }";
    css_stylesheet_append_data(sheet, (const uint8_t*)css, strlen(css));
    css_stylesheet_data_done(sheet);
    
    // We want to see if the bytecode has the gap length set
    css_rule *rule = NULL;
    /* This requires digging into private structures. We just run ctest instead to see if crash. */
}
