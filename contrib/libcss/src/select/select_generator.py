...
class CSSGroup:
        ...
        for p in sorted(self.props, key=(lambda x: x.name)):
            ...
            if p.has_calc:
                ...
                for value in p.values:
                    if value.calc and value.bits is not None:
                        unit_name = value.bits['name'] + value.suffix
                        calc_field_for_unit[unit_name] = p.name + value.suffix

                t.append('if ({}) {{'.format(calc_cond))
                t.indent(1)
                for i, v in enumerate(list(reversed(shift_list))):
                    unit_name = v[0]
                    calc_field = calc_field_for_unit.get(unit_name)
                    if calc_field is None:
                        continue
                    t.append('if ((orig_bits & 0x{:x}) >> {} == CSS_UNIT_CALC) {{'.format(v[2], v[1]))
                    t.indent(1)
                    t.append('lwc_string_unref(style->i.{}.calc);'.format(calc_field))
                    t.indent(-1)
                    t.append('}')
                t.indent(-1)
                t.append('}')
            ...
            for v in p.values:
                ...
                if v.name == 'string':
                    ...
                    if p.has_calc:
                        unit_var = v.bits['name'] + v.suffix if v.bits is not None else 'unit'
                        t.append('if ({} == CSS_UNIT_CALC) {{'.format(unit_var))
    ...
    def print_destroy(self, t, p):
        if p.has_calc:
            ...
            for v in p.values:
                if v.calc:
                    args.append('(css_fixed_or_calc)0')
                else:
                    args.append(v.defaults if v.defaults and v.defaults != 'NULL' else '0')
                if v.bits is not None:
                    args.append(v.bits['defaults'])
            t.append("set_{}({});".format(p.name, ', '.join(args)))
        else:
