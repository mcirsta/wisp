...
css_error css_computed_style_destroy(css_computed_style *style)
{
    ...
    /* Free custom properties map */
    if (style->custom_props != NULL) {
        css__custom_property_map_destroy(style->custom_props);
    }

    /* Free deferred var() list */
    if (style->deferred_vars != NULL) {
        css__deferred_var_list_destroy(style->deferred_vars);
    }
...
css_error css__computed_style_clone(const css_computed_style *orig, css_computed_style **clone_out)
{
    ...
    /* Clone custom properties if present */
    if (orig->custom_props != NULL) {
        error = css__custom_property_map_clone(orig->custom_props, &clone->custom_props);
        if (error != CSS_OK) {
            css_computed_style_destroy(clone);
            return error;
        }
    ...
    const css_computed_style *restrict child, const css_unit_ctx *unit_ctx, css_computed_style **restrict result)
{
    ...
    /* Inherit custom properties from parent, then overlay child's */
    if (parent->custom_props != NULL) {
        error = css__custom_property_map_clone(parent->custom_props, &composed->custom_props);
        if (error != CSS_OK) {
            css_computed_style_destroy(composed);
            return error;
        }
#ifdef DEBUG_CSS_VAR
        fprintf(stderr, "[CSS-VAR] Inherited %u custom properties from parent\n", parent->custom_props->count);
#endif
    }
    if (child->custom_props != NULL) {
        if (composed->custom_props == NULL) {
            error = css__custom_property_map_clone(child->custom_props, &composed->custom_props);
        } else {
            error = css__custom_property_map_merge(composed->custom_props, child->custom_props);
        }
        if (error != CSS_OK) {
            css_computed_style_destroy(composed);
            return error;
...
#ifdef DEBUG_CSS_VAR
        fprintf(stderr, "[CSS-VAR] Merged %u custom properties from child\n", child->custom_props->count);
#endif
    }

    /* Resolve deferred var() entries now that custom_props are complete */
    if (child->deferred_vars != NULL && child->deferred_vars->count > 0) {
#ifdef DEBUG_CSS_VAR
        fprintf(stderr, "[CSS-VAR-COMPOSE] Resolving %u deferred var() entries\n", child->deferred_vars->count);
#endif

        for (uint32_t i = 0; i < child->deferred_vars->count; i++) {
            css_deferred_var_entry *entry = &child->deferred_vars->entries[i];
#ifdef DEBUG_CSS_VAR
            fprintf(stderr, "[CSS-VAR-COMPOSE] Entry %u: prop=%u, %u tokens\n", i, entry->prop_idx, entry->token_count);
#endif

            /* Log tokens for debugging */
            for (uint32_t j = 0; j < entry->token_count; j++) {
                css_deferred_token *tok = &entry->tokens[j];
#ifdef DEBUG_CSS_VAR
                fprintf(stderr, "[CSS-VAR-COMPOSE]   Token %u: type=%u, data='%s'\n", j, tok->type,
                    tok->idata ? lwc_string_data(tok->idata) : "(null)");
#endif
            }

            /* TODO: Implement actual var() substitution:
             * 1. Walk tokens, detect var() function
             * 2. Look up variable name in composed->custom_props
             * 3. Substitute with value
             * 4. Parse and apply to composed style
             */
        }
    }
