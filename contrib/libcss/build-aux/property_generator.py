...
class MultiFileGenerator:
    ...
    def generate_parse_handler_table(self):
        """Generate opcode-indexed parse handler table for var() resolution.
        
        Produces an array css_property_parse_handlers[CSS_N_PROPERTIES] that maps
        each CSS_PROP_* opcode to its css_prop_handler parse function.
        """
        lines = self._header("PARSE_HANDLER_TABLE",
            "Parse handler table indexed by CSS_PROP_* opcode.\n"
            " * Used during var() resolution to look up the property parser by opcode.")
        lines.append('')
        lines.append('#include "parse/properties/properties.h"')
        lines.append('')
        lines.append('static const css_prop_handler')
        lines.append('css_property_parse_handlers[CSS_N_PROPERTIES] = {')
        
        for idx, prop_info in enumerate(self.dispatch_order):
            name = prop_info['name']
            handler = f"css__parse_{name}"
...
def main():
    ...
    parse_handler_table_out = os.path.join(output_dir, 'parse_handler_table.inc')
    ...
    for out_file, gen_func in [
        ...
        (parse_handler_table_out, generator.generate_parse_handler_table),
    ]:
